...
public: //PCR
    // Store grayscale image for all sensor types
    cv::Mat mImGray;

    // Accessor method for grayscale image
    inline const cv::Mat& GetGrayImage() const { return mImGray; }

    // Method to set grayscale image 
    inline void SetGrayImage(const cv::Mat& im) { mImGray = im.clone(); }

    bool HasGrayImage() const {
        return !mImGray.empty();
    }
