    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    mbEvaluateFiltering = true;
    mFilterEvaluationPath = "./filter_evaluation/";
    mEvalFrameInterval = 10;  // Evaluate every 10 frames

    // Initialize point cloud filter with CUDA support
    mpPointCloudFilter = std::make_unique<PointCloudFilter>();
    
    // Print CUDA status
    std::cout << "CUDA status for PointCloudFilter: " 
              << (mpPointCloudFilter->mCudaEnabled ? "ENABLED" : "DISABLED") << std::endl;
    ...
    mpPointCloudFilter->ResetFilterStats();
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.SetGrayImage(mImGray);
    std::cout << "DEBUG: Storing grayscale image for frame " << mCurrentFrame.mnId 
              << " size: " << mImGray.size() << std::endl;
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.SetGrayImage(mImGray);//PCR
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.SetGrayImage(mImGray);//PCR
    std::cout << "DEBUG: Storing grayscale image for monocular frame " << mCurrentFrame.mnId 
              << " size: " << mImGray.size() << std::endl;
...
void Tracking::Track()
{
    ...
    if(mState!=NO_IMAGES_YET)
    {
        // Add or modify the PointCloudFilter section with CUDA-aware implementation
        if(mpPointCloudFilter && mpPointCloudFilter->IsInitialized()) {
            std::cout << "\nDebug Filter Conditions:" << std::endl;
            std::cout << "Current State: " << mState << " (OK=" << OK << ")" << std::endl;
            std::cout << "Frame Points: " << mCurrentFrame.N << std::endl;
            std::cout << "Last KeyFrame ID: " << mnLastKeyFrameId << std::endl;
            std::cout << "KeyFrames in Map: " << mpAtlas->GetCurrentMap()->KeyFramesInMap() << std::endl;
            
            // Start filtering after we have some keyframes and are in tracking state
            bool shouldFilter = (mState == OK) &&                     // We're in OK state
                            (mpAtlas->GetCurrentMap()->KeyFramesInMap() > 2) &&  // Have some keyframes
                            (mCurrentFrame.N > 30);                 // Frame has enough points for filtering
            
            if(shouldFilter) {
                std::cout << "Starting point cloud filtering with CUDA acceleration..." << std::endl;
                std::cout << "Frame ID: " << mCurrentFrame.mnId << std::endl;
                
                // Store original outlier states before filtering
                std::vector<bool> originalOutliers = mCurrentFrame.mvbOutlier;
                
                // Process asynchronously if possible to reduce latency in main thread
                auto start = std::chrono::high_resolution_clock::now();
                
                if (mpPointCloudFilter->mCudaEnabled) {
                    // Using CUDA - run in current thread for now
                    // Could be made async in future versions with proper synchronization
                    mpPointCloudFilter->FilterFrame(mCurrentFrame);
                } else {
                    // CPU-only - process in current thread
                    mpPointCloudFilter->FilterFrame(mCurrentFrame);
                }
                
                auto end = std::chrono::high_resolution_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
                std::cout << "Total filtering time: " << duration.count() << "ms" << std::endl;
                
                // Evaluate filtering performance
                if(mbEvaluateFiltering) {
                    mpPointCloudFilter->EvaluateFiltering(originalOutliers, mCurrentFrame.mvbOutlier, mCurrentFrame);
                    
                    // Save results periodically
                    if(mCurrentFrame.mnId % mEvalFrameInterval == 0) {
                        std::string filename = mFilterEvaluationPath + "filter_eval_" + 
                                            std::to_string(mCurrentFrame.mnId) + ".txt";
                        mpPointCloudFilter->SaveConfusionMatrix(filename);
                        
                        // Print confusion matrix to console but DON'T reset the stats
                        PointCloudFilter::FilterStats stats = mpPointCloudFilter->GetFilterStats();
                        std::cout << "\nCumulative Confusion Matrix:" << std::endl;
                        std::cout << "┌───────────────┬──────────────┬──────────────┐" << std::endl;
                        std::cout << "│               │ Pred Dynamic │ Pred Static  │" << std::endl;
                        std::cout << "├───────────────┼──────────────┼──────────────┤" << std::endl;
                        std::cout << "│ True Dynamic  │ " << std::setw(12) << stats.truePositives 
                                << " │ " << std::setw(12) << stats.falseNegatives << " │" << std::endl;
                        std::cout << "├───────────────┼──────────────┼──────────────┤" << std::endl;
                        std::cout << "│ True Static   │ " << std::setw(12) << stats.falsePositives 
                                << " │ " << std::setw(12) << stats.trueNegatives << " │" << std::endl;
                        std::cout << "└───────────────┴──────────────┴──────────────┘" << std::endl;
                        
                        // Calculate and print metrics
                        float precision = (stats.truePositives + stats.falsePositives > 0) ? 
                                float(stats.truePositives) / (stats.truePositives + stats.falsePositives) : 0.0f;
                        float recall = (stats.truePositives + stats.falseNegatives > 0) ? 
                                float(stats.truePositives) / (stats.truePositives + stats.falseNegatives) : 0.0f;
                ...
                std::cout << "Skipping filtering - conditions not met" << std::endl;
            }
        }
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD))
                    {
                        ...
                        {
                            mState = LOST;
                            Verbose::PrintMess("TrTrack()ack Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK=false;
                        }
                    }
                    else
